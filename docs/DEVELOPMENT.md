...
└────────┬────────┘     └────────┬────────┘
         ...
         ▼                       ▼
┌─────────────────┐  ┌─────────────────────┐  ┌─────────────────┐
│ app_protocol.c  │  │ example_echo_hal.c  │  │ app_logview.c   │
│ (信号/配置处理) │  │ (示例 HAL)         │  │ (日志显示)      │
└─────────────────┘  └────────┬────────────┘  └─────────────────┘
                              │
                              ▼
...
| `example_echo_hal.c/h` | Echo 示例 HAL（send, log），实际应用需替换 |
| `example_echo.c/h` | Echo 协议示例（回环），实际应用需替换 |
...
**注意：** 协议层通过 HAL 层操作串口，不直接依赖 `serial.h`。HAL 层需根据协议需求自定义（示例见 `example_echo_hal.h`）。
